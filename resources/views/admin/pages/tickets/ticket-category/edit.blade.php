        ...
        <div class="card">
            <div class="p-6">
                <div class="flex items-center justify-between mb-6">
                    <h3 class="card-title">Edit Ticket Category</h3>
                    <a href="{{ url('admin/ticket-category') }}" class="flex rounded-sm overflow-hidden">
                        <button
                ...
                <form action="{{ url('admin/ticket-category', $getTicketCategory->id) }}" method="POST" enctype="multipart/form-data">
                    <div class="grid lg:grid-cols-2 gap-5">
                        @csrf
                        @method('PUT')
                        <div>
                            <label for="name" class="block mb-2">Event Ticket Name</label>
                            <input type="text" name="name" class="form-input" value="{{$getTicketCategory->name}}" id="name" required>
                        </div>
